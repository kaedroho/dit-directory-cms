...
from find_a_supplier.models import FindASupplierApp, FindASupplierLocaleRootPage


class Command(BaseCommand):
    @transaction.atomic
    def restructure_existing_pages(self):
        if FindASupplierLocaleRootPage.objects.exists():
            print("A locale root page already exists")
            return

        invest_app_root = FindASupplierApp.objects.get()

        print("Creating locale root")
        default_locale = Locale.objects.default()
        locale_root = invest_app_root.add_child(instance=FindASupplierLocaleRootPage(
    ...
    def handle(self, **options):
        ...
        page_models = [
            ...
            if model._meta.app_label == 'find_a_supplier' and issubclass(model, TranslatablePageMixin)
        ]
